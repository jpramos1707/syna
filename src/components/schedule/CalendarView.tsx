...
export function CalendarView({ posts, onAddPost, onSelectPost, onSelectDay }: CalendarViewProps) {
        ...
        {days.map((day) => {
          ...
          return (
            <div
              key={day.toISOString()}
              className={cn(
                "calendar-day min-h-[100px] flex flex-col group cursor-pointer",
                isCurrentDay && "calendar-day-today",
                !isSameMonth(day, currentMonth) && "opacity-50"
              )}
              onClick={(e) => {
                // Se clicou no botão + ou em um post, não abre o modal do dia
                if ((e.target as HTMLElement).closest('button') || (e.target as HTMLElement).closest('[data-post-item]')) {
                  return;
                }
                onSelectDay?.(day);
              }}
              ...
              <div className="flex-1 space-y-1 overflow-hidden">
                {dayPosts.slice(0, 3).map((post) => (
                  <div
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      // Quando clica no post, abre o modal de visualização do dia
                      onSelectDay?.(day);
                    }}
                    className={cn(
                ...
                {dayPosts.length > 3 && (
                  <div 
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      // Quando clica em "+X mais", abre o modal de visualização do dia
                      onSelectDay?.(day);
                    }}
                  >
