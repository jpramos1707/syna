...
export function PostModal({ open, onOpenChange, clientId, clientName = "Cliente", existingPost, initialDate }: PostModalProps) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (open && existingPost) {
      ...
      // Store script in testing_hypothesis when post_type is video or reels
      setScript(
        (existingPost.post_type === "video" || existingPost.post_type === "reels")
          ? (existingPost.testing_hypothesis || "")
          : ""
      );
  ...
  const handleFinalize = async () => {
    ...
    const postData = {
      ...
      created_by: user?.id,
      // Store script in testing_hypothesis when post_type is video or reels
      // Otherwise, keep existing testing_hypothesis or set to null if new post
      testing_hypothesis: (postType === "video" || postType === "reels")
        ? (script || null)
        : (existingPost?.testing_hypothesis || null),
    };
        ...
        <Tabs defaultValue="basic" className="w-full">
          ...
          <div className="overflow-y-auto max-h-[60vh] mt-4 pr-2 scrollbar-thin">
            <TabsContent value="basic" className="space-y-6 animate-fade-in">
              ...
              {(postType === "video" || postType === "reels") && (
                <div className="space-y-2">
                  <Label>Roteiro do Vídeo</Label>
                  <Textarea
                    placeholder="Digite o roteiro do vídeo aqui..."
